...
class MainWindow(QtWidgets.QWidget):
	...
	def eventFilter(self, source, event):
		...
		if (event.type() == QtCore.QEvent.ContextMenu and source is self.tree or event.type() == QtCore.QEvent.ContextMenu and source is self.tab1FolderView):
			...
			menu.addAction("Check For ColorManagement in Children")
			...
			if not action == None:
				...
				if action.text() == "Check For ColorManagement in Children":
					print("Running CM Check for %s" % node.GetName())
					for cur_node in node.getAllChildren():
						QtCore.QThreadPool.globalInstance().start(CheckForColorManagementClass(cur_node))
	...
	def closeEvent(self, event):
		...
		file_util.save_json(self.node_dict_path, self.all_node_list)  # Save nodes
