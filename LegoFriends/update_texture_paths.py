...
def updateStandInBasedOnOrigStructure(start_path=None):
    # Define the directory to check
    if not start_path:
        scene_file_path = cmds.file(query=True, l=True)[0]
        start_path = os.path.dirname(scene_file_path)


    # Get all file nodes
    file_nodes = cmds.ls(type="aiStandIn")
    print('Found aiStandIn nodes: %s' % file_nodes)
    print('\n   ... Indexing file nodes ...')
    ...
    for file_node in file_nodes:
        # Get the file path from the node
        file_path = cmds.getAttr(file_node + ".dso")
        if file_path:
            file_name = os.path.basename(file_path)
            if file_name in reference_dictionary.keys():
                reference_dictionary[file_name].append(file_node)
    ...
    for root, dirs, files in os.walk(start_path):
        for file in files:
            ...
            # Updating the texture file path
            for node in reference_dictionary[file]:
                cmds.setAttr(node + ".dso", os.path.join(root, file), type='string')
                number_of_spaces = 50 - len(node)
                spaces = ' ' * number_of_spaces
                print(' > ' + node + spaces + '  -->  ' + os.path.join(root, file))
...
def updateGPUBasedOnOrigStructure(start_path=None):
    # Define the directory to check
    if not start_path:
        scene_file_path = cmds.file(query=True, l=True)[0]
        start_path = os.path.dirname(scene_file_path)
    ...
    # Get all file nodes
    file_nodes = cmds.ls(type="gpuCache")
    ...
    for file_node in file_nodes:
        # Get the file path from the node
        file_path = cmds.getAttr(file_node + ".cacheFileName")
        if file_path:
            file_name = os.path.basename(file_path)
            if file_name in reference_dictionary.keys():
                reference_dictionary[file_name].append(file_node)
    ...
    for root, dirs, files in os.walk(start_path):
        for file in files:
            ...
            # Updating the texture file path
            for node in reference_dictionary[file]:
                cmds.setAttr(node + ".cacheFileName", os.path.join(root, file), type='string')
                number_of_spaces = 50 - len(node)
                spaces = ' ' * number_of_spaces
                print(' > ' + node + spaces + '  -->  ' + os.path.join(root, file))
...
def updateTextureBasedOnOrigStructure(start_path=None):
    if not start_path:
        # Define the directory to check
        scene_file_path = cmds.file(query=True, l=True)[0]
        start_path = os.path.dirname(scene_file_path)
