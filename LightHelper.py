...
# CIM.dropCachedImports("Maya_Functions.vray_util_functions")
...
#TODO Rename also includes renaming file and thumb with new prefix.

#Maya:
#TODO Export group without namespace!!

#UI:
#TODO Low prio Scale UI thumbnail for easy viewing.
#Pix 160x90 = save as thumbnail, first save the old thumbnail as "name_Big".
...
class UIUtils(object):
    ...
    def checkSourceImage(self, img_path, img_name): #Not used anymore
        """This method evaluates if source image is too big and makes a smaller copy if true"""

        return  # Keep return statement here until this method is fixed!

        img_name = img_name + "_Thumb.png"
        img_path = img_path.replace("//", "/").replace("/Cam_Dict.json", "/Thumbs")
        old_img_url = img_path + "/" + img_name

        # If file doesn't exist, return to function call
        if not os.path.isfile(path=old_img_url):
            return -1

        # Load image
        img = QtGui.QImage(old_img_url)

        # If image size is greater than 160x90:
        if img.width() > 160:  # or img.height() > 90

            logger.info("\nA large thumbnail was found in use. Copying and resizing now:")

            # Get original name and add prefix _Big
            new_img_name = "Big_" + img_name
            new_img_url = img_path + "/" + new_img_name

            # Copy original image and rename with prefix 'Big_': Make 'Backup'
            # If 'new_img_url' already exists in dir, shutil will override this file
            shutil_copyfile(old_img_url, new_img_url)

            # Resize original down to 160xXX pixels by updating itself with transformed copies of itself
            # img = img.scaledToWidth(160)  # Not needed, '.scaledTo...()' scales with aspect ratio
            img = img.scaledToWidth(160)
            # Save Image with QtGui.QImage
            img.save(old_img_url)

            logger.debug( "    +'{0}' - copied to - {1}\n".format(img_name, new_img_name))
...
class MainWindow(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.base_folder = CC.get_light_export_folder() #cfg_util.CreatePathFromDict(cfg.project_paths["light_export_folder"])
        # self.save_path = "%s/Cam_Dict.json" % self.base_folder
    ...
    def CreateWindow(self):
        ...
        self.pick_light_setup_bttn = QtWidgets.QPushButton("Pick Light Setup Group")
        self.pick_light_setup_bttn.clicked.connect(self.PickLightSetup)

        self.pick_light_setup_combo = QtWidgets.QComboBox()
        self.pick_light_setup_combo.setMinimumWidth(200)
        self.pick_light_setup_combo.currentTextChanged.connect(self.PickLightSetupComboChange)
        ...
        self.remove_namespace_bttn.clicked.connect(self.remove_namespace_call)#self.LF.RemoveNamespace)
        self.remove_namespace_bttn.setToolTip("Removes namespace from selected light group")

        self.create_sphere_bttn = QtWidgets.QPushButton("Create VraySphere")
        self.create_sphere_bttn.clicked.connect(self.LF.createSphere)
        ...
        # Buttons for Mia highlights
        self.locators_to_sun_btn = QtWidgets.QPushButton("Locators to Sun")
        self.locators_to_sun_btn.setMaximumWidth(140)
        self.locators_to_sun_btn.clicked.connect(self.moveLocatorsToSun)
        self.locators_to_selected_btn = QtWidgets.QPushButton("Locators to Selected")
        self.locators_to_selected_btn.setMaximumWidth(140)
        self.locators_to_selected_btn.clicked.connect(self.moveLocatorsToSelected)
        self.select_locators_btn = QtWidgets.QPushButton("Select Locators")
        self.select_locators_btn.setMaximumWidth(140)
        self.select_locators_btn.clicked.connect(self.selectLocators)

        self.import_z_bttn = QtWidgets.QPushButton("Import Z Planes")
        self.import_z_bttn.clicked.connect(self.LF.ImportZplanes)
        # self.re_z_bttn = QtWidgets.QPushButton("Reconnect Z Planes")
        # self.re_z_bttn.clicked.connect(self.LF.ReconnectZplanesToVray)

        self.menu_bar = QtWidgets.QMenuBar()

        self.menu_functions = QtWidgets.QMenu("Functions")

        # self.pick_cam_action = QtWidgets.QAction("Pick Render Camera",self.menu_bar)
        # self.pick_cam_action.triggered.connect(self.LF.ReAlignLocator) #FIX THIS
        # self.menu_functions.addAction(self.pick_cam_action)
        #
        # self.pick_light_setup_action = QtWidgets.QAction("Pick LightSetup", self.menu_bar)
        # self.pick_light_setup_action.triggered.connect(self.LF.ReAlignLocator)  # FIX THIS
        # self.menu_functions.addAction(self.pick_light_setup_action)


        self.rebuild_constraints = QtWidgets.QAction("Rebuild LightSetup Constraints", self.menu_bar)
        self.rebuild_constraints.triggered.connect(self.LF.rebuildConstraints) #FIX THIS
        self.menu_functions.addAction(self.rebuild_constraints)

        self.align_to_cam = QtWidgets.QAction("ReAlign To Camera",self.menu_bar)
        self.align_to_cam.triggered.connect(self.LF.ReAlignLocator)
        self.menu_functions.addAction(self.align_to_cam)

        self.menu_functions.addSeparator()

        self.rebuild_message = QtWidgets.QAction("Rebuild Message Attr on LightSetup",self.menu_bar)
        self.rebuild_message.triggered.connect(self.LF.RebuildMessageAttr)
        self.menu_functions.addAction(self.rebuild_message)

        self.connect_dome_ac = QtWidgets.QAction("Link Dome Light -> LightSetup",self.menu_bar)
        self.connect_dome_ac.triggered.connect(self.LF.ConnectToDome)
        self.menu_functions.addAction(self.connect_dome_ac)

        self.connect_loc_ac = QtWidgets.QAction("Link Cam Locator -> LightSetup",self.menu_bar)
        self.connect_loc_ac.triggered.connect(self.LF.ConnectToLocator)
        self.menu_functions.addAction(self.connect_loc_ac)
        ...
        self.sort_by_loc = QtWidgets.QAction("Sort By Closest to Camera",self.menu_bar)
        self.sort_by_loc.triggered.connect(self.SortByLocation)
        self.menu_ui.addAction(self.sort_by_loc)
        ...
        self.button_layout.addWidget(self.create_sphere_bttn)

        self.button_layout.addWidget(self.import_z_bttn)

        self.pick_layout = QtWidgets.QHBoxLayout()
        self.pick_layout.addWidget(self.pick_light_setup_bttn)
        self.pick_layout.addWidget(self.pick_light_setup_combo)
        ...
        #self.category_layout.setAlignment(QtCore.Qt.AlignLeft)
        self.category_layout.addStretch(1)
        #self.locator_button_layout = QtWidgets.QHBoxLayout()
        self.category_layout.addWidget(self.locators_to_sun_btn)
        self.category_layout.addWidget(self.locators_to_selected_btn)
        self.category_layout.addWidget(self.select_locators_btn)
        ...
        # self.thumb_view.clicked.connect(self.folderQuickviewClicked)

        self.main_layout.addLayout(self.button_layout)
        self.main_layout.addLayout(self.pick_layout)
        self.main_layout.addLayout(self.category_layout)
        #self.main_layout.addLayout(self.locator_button_layout)
    ...
    def remove_namespace_call(self):
        ...
        # Updates sky node if relevant
        if self.LF.sky:
            if not cmds.objExists(self.LF.sky):
                self.LF.sky = self.LF.findSky(self.LF.light_setup)

    def moveLocatorsToSun(self):
        self.LF.elevateHighlightAims(height = 1000)

    def moveLocatorsToSelected(self):
        self.LF.moveLocatorsToSelectedObject()

    def selectLocators(self):
        self.LF.selectHighlightLocators()


    def PickCameraComboChange(self):
        if self.pick_camera_combo.currentText():
            self.LF.setRenderCam(self.pick_camera_combo.currentText())
    ...
    def initiateCamAndSetupComboBox(self):
        setup_list = self.LF.FindLightGroupInScene(all=True)
        self.pick_light_setup_combo.clear()
        self.pick_light_setup_combo.addItems(setup_list)
        self.PickCamera()
        # cam_combo_list = self.LF.getAllCamerasInScene()
        # self.pick_camera_combo.clear()
        # self.pick_camera_combo.addItems(cam_combo_list)

    def PickLightSetup(self):
        picked = self.LF.FindLightGroupInScene()
        setup_list = self.LF.FindLightGroupInScene(all=True)
        self.pick_light_setup_combo.clear()
        self.pick_light_setup_combo.addItems(setup_list)
        if picked:
            self.pick_light_setup_combo.setCurrentText(picked)

    def PickLightSetupComboChange(self):
        if self.pick_light_setup_combo.currentText():
            self.LF.setLightSetup(self.pick_light_setup_combo.currentText())
    ...
    def UpdateCamLoc(self, cur_node):
        text, ok = QtWidgets.QInputDialog().getText(self, "Update Position: ", "example: Clearing_Cam_04:", QtWidgets.QLineEdit.Normal, "")
        if ok and text:
            node_name = text
        else:
            logger.info("Cancel creation because no name was given")
        ...
        cur_node.SetCamLoc(node_name)

    def CreateLightSetup(self):
        self.LF.SetupLightGroup()
    ...
    def SortByLocation(self):
        current_vector = self.LF.GetCurrentLocation()
        if current_vector:
            self.sort_nodes = sorted(self.sort_nodes, key=lambda x: self.ReturnLocationDifference(current_vector,x.GetCamLoc()))
            self.thumb_model = ThumbModel(self.sort_nodes, self)
            self.thumb_view.setModel(self.thumb_model)

    def ReturnLocationDifference(self, goal_vec, cur_vec):
        from math import sqrt
        dist = sqrt(((goal_vec[0]-cur_vec[0])**2)+((goal_vec[1]-cur_vec[1])**2)+((goal_vec[2]-cur_vec[2])**2))
        return dist

    #TODO Function to save pixmap as png in a smaller scale.
