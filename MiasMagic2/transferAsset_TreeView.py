...
class Node(object):
    ...
    def generateNewAssetInfoDict(self):
        self.generateInfoDict()
        self.new_info_dict = self.info_dict.copy()
        if self.asset_type == "Char" and self.asset_category == "Module":
            self.new_info_dict["asset_type"] = "RigModule"
            if "eye" in self.name.lower():
                self.new_info_dict["asset_category"]= "Eye"
            elif "mouth" in self.name.lower():
                self.new_info_dict["asset_category"]="Mouth"
        if self.asset_type == "SetDress":
            self.new_info_dict["asset_type"] = "Setdress"
        if self.asset_category in ["Bushs", "Trees", "Grounds"]:
            self.new_info_dict["asset_category"] = "Forest"

    def UpdateAssetTextureAndRef(self):
        self.generateNewAssetInfoDict()
        old_path = CC.old.get_asset_base_path(asset_type=self.asset_type, asset_name=self.name,
                                              asset_category=self.asset_category)
        new_path = CC.get_asset_base_path(**self.new_info_dict)
        replace_list = []
        ...
        replace_list.append(('/Char/Module/EyeRainbowGuy/02_Ref/EyeRainbowGuy_Model_Ref.mb', '/RigModule/Eye/EyeRainbowGuy/02_Ref/EyeRainbowGuy_Rig_Ref.mb'))
        replace_list.append(('/Char/Module/EyeSmallDragon/02_Ref/EyeSmallDragon_Model_Ref.mb', '/RigModule/Eye/EyeSmallDragon/02_Ref/EyeSmallDragon_Rig_Ref.mb'))
        replace_list.append(('/Char/Module/MouthA/02_Ref/MouthA_Model_Ref.mb', 'RigModule/Mouth/MouthA/02_Ref/MouthA_Model_Ref.mb'))

        replace_list.append((CC.old.get_base_path(),CC.get_base_path()))
        replace_list.append(("/%s/" % self.asset_type,"/%s/" % self.new_info_dict["asset_type"]))
        replace_list.append(("/%s/" % self.asset_category, "/%s/" % self.new_info_dict["asset_category"]))
        replace_list.append(("/%s/" % self.name, "/%s/" % self.name))
    ...
    def CopyAssetToNewAsset(self):
        rigged_assets = ['BushE', 'FaceFlowerA', 'MushroomA', 'MushroomB', 'MushroomC', 'RockA', 'RockB', 'RockC',
        ...
        if self.asset_type == "Char" and self.asset_category =="Module":
            ...
            new_asset_type = "RigModule"
            if "eye" in self.name.lower():
                new_asset_category = "Eye"
            elif "mouth" in self.name.lower():
                new_asset_category ="Mouth"
            new_path = CC.get_asset_base_path(asset_type=new_asset_type, asset_category=new_asset_category,
                                                  asset_name=self.name)

            new_dict["asset_type"] = new_asset_type
            new_dict["asset_category"] = new_asset_category
            new_dict["asset_name"] = self.name

            from AssetFunctions import CreateAsset
            new_asset = CreateAsset(new_dict)
            new_asset.Run()

            from MiasMagic2.transfer_functions import transferTextureFolder,transferDesignFolder
            transferTextureFolder(asset_info=self.info_dict, new_asset_info=new_dict)
            transferDesignFolder(asset_info=self.info_dict, new_asset_info=new_dict)

        new_dict = self.info_dict.copy()
        if self.asset_type == "SetDress":
            new_dict["asset_type"] = "Setdress"

        if self.asset_category in ["Bushs", "Trees", "Grounds"]:
            new_dict["asset_category"] = "Forest"

        if self.asset_type =='Char' and not self.asset_category =="Module":
            from MiasMagic2.transfer_functions import transferCharacter
            transferCharacter(self.info_dict)

        elif self.asset_type == 'Prop':
            from MiasMagic2.transfer_functions import transferProp
            transferProp(self.info_dict)

        elif self.asset_type.lower() == 'setdress':
            if self.name in rigged_assets:
                from MiasMagic2.transfer_functions import transferRiggedSetdress
                transferRiggedSetdress(new_dict, self.info_dict)

            else:
                from MiasMagic2.transfer_functions import transferSetdress
                transferSetdress(new_dict, self.info_dict)

        elif self.asset_type == 'Set':
            from MiasMagic2.transfer_functions import transferSet
            transferSet(new_dict)

        print("Done transferring " + self.name)
...
class MainWindow(QtWidgets.QWidget):
    ...
    def createWindow(self):
        ...
        self.add_button = QtWidgets.QPushButton("Transfer Asset")
        ...
        self.add_button.clicked.connect(lambda: self.transferSelectedAssets())
    ...
    def transferSelectedAssets(self):
