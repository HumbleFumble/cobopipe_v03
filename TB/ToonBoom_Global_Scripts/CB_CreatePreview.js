...
//        var pythonPath = "T:/_Executables/python/Python310/python.exe";
    ...
    function MakeSlate_NoServerPython(input_path, soundPath,output_path, shot_name, crop, width, height,x_cord,y_cord){
        ...
        MessageLog.trace("ffmpeg -i "+soundPath + " -i " + input_path + " -filter_complex [0:a]asetpts=expr=PTS-STARTPTS[s0];[1:v]crop=h=" + height + ":w=" + width + ":x=" + x_cord.toFixed(1) + ":y=" + y_cord.toFixed(1) + "[s1];[s1]drawtext=fontcolor=white:fontfile=Arial:fontsize=24:shadowcolor=black:shadowx=2:shadowy=2:text="+ shot_name +":x=w-(text_w+20):y=20[s2];[s2]drawtext=fontcolor=white:fontfile=Arial:fontsize=24:shadowcolor=black:shadowx=1.5:shadowy=1.5:start_number=1:text=%{eif\\\\:n\\\\:d\\\\:5}:x=w-(text_w+20):y=50[s3];[s3]drawtext=fontcolor=white:fontfile=Arial:fontsize=24:shadowcolor=black:shadowx=2:shadowy=2:text=" +timestamp +":x=w-(text_w+20):y=h-(text_h+20)[s4] -map [s0] -map [s4] -acodec pcm_s16le -pix_fmt yuv420p " + output_path +" -y");
        proc_string = "ffmpeg -i "+soundPath + " -i " + input_path + " -filter_complex [0:a]asetpts=expr=PTS-STARTPTS[s0];[1:v]crop=h=" + height + ":w=" + width + ":x=" + x_cord.toFixed(1) + ":y=" + y_cord.toFixed(1) + "[s1];[s1]drawtext=fontcolor=white:fontfile=Arial:fontsize=24:shadowcolor=black:shadowx=2:shadowy=2:text="+ shot_name +":x=w-(text_w+20):y=20[s2];[s2]drawtext=fontcolor=white:fontfile=Arial:fontsize=24:shadowcolor=black:shadowx=1.5:shadowy=1.5:start_number=1:text=%{eif\\\\:n\\\\:d\\\\:5}:x=w-(text_w+20):y=50[s3];[s3]drawtext=fontcolor=white:fontfile=Arial:fontsize=24:shadowcolor=black:shadowx=2:shadowy=2:text=" +timestamp +":x=w-(text_w+20):y=h-(text_h+20)[s4] -map [s0] -map [s4] -acodec pcm_s16le -pix_fmt yuv420p " + output_path +" -y";
