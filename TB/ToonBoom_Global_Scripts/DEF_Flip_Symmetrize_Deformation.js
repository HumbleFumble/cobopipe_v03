...
function private_functions()
{
		...
		if( pref.reset || pref.fulcrum !== "origin" )
		{
			...
			for( var n = 0; n < numNodes; n++ )
			{
				if( pref.fulcrum !== "origin" )
				{
					...
					switch( pref.mode )
					{				
						case "symmetry": this.addKeyframe( defNodes[n].node, attr, f, val + offsetVal /2 ); break;
						default: this.addKeyframe( defNodes[n].node, attr, f, val + offsetVal ); break;
					}						
				}				
				if( pref.reset )
		...
		if( pref.reset || pref.fulcrum !== "origin" )
		{
			...
			for( var n = 0; n < numNodes; n++ )
			{
				if( pref.fulcrum !== "origin" )
				{
					...
					switch( pref.mode )
					{				
						case "flip": this.addKeyframe( defNodes[n].node, attr, f, val + offsetVal ); break;
						default: this.addKeyframe( defNodes[n].node, attr, f, val + offsetVal /2 ); break;
					}						
				}				
				if( pref.reset )
					this.applyToResting( defNodes[n].node, f )	
	...
	{		
		...
		for( var n = 0; n < numNodes; n++ )
		{
			...
			this.addKeyframe( defNodes[n].node, attr, f, -pos1 );
			this.addKeyframe( defNodes[n].node, attr2, f, pos2 );
			...
			{					
				...
				this.addKeyframe( defNodes[n].node, "orientation0", f, flipped0 );
				...
				this.addKeyframe( defNodes[n].node, "orientation1", f, flipped1 );			
				...
				this.addKeyframe( defNodes[n].node, "length0", f, length0 );
				this.addKeyframe( defNodes[n].node, "length1", f, length1 );			
			}
			else   	// offset nodes
			{						
				...
				this.addKeyframe( defNodes[n].node, "orientation", f, flippedOriernt );
			}					
		}	
	};
	...
	this.flipBoneAndCurve = function( nodeObj, attr, f, rotate180, pref, isFirstBone )
	{
		...
		if( nodeObj.type == "CurveModule" )     // curve mode curve nodes
		{
			...
			this.addKeyframe( nodeObj.node, "orientation0",  f, -orient0 );
			this.addKeyframe( nodeObj.node, "orientation1", f, -orient1 );								
			
			// set keys as is
			var length0 = node.getAttr( nodeObj.node, f, "length0" ).doubleValue();
			var length1 = node.getAttr( nodeObj.node, f, "length1" ).doubleValue();
			this.addKeyframe( nodeObj.node, "length0", f, length0 );
			this.addKeyframe( nodeObj.node, "length1", f, length1 );			
		}
		else   								// bone and offset nodes
		{
			...
			var flippedOriernt = this.getProjectedAngle( orient, rotate180 );
			this.addKeyframe( nodeObj.node, "orientation", f, flippedOriernt );			
			...
			if( nodeObj.type == "BendyBoneModule" )
			{
				...
				this.addKeyframe( nodeObj.node, "radius", f, radius );
				this.addKeyframe( nodeObj.node, "bias", f, bias );
				this.addKeyframe( nodeObj.node, "length", f, length );			
			}
		}
	};
	...
	this.swapSelfHandlePos = function( nodeObj, f, wing1, pref )
	{
		...
		switch( pref.mode )
		{				
			case "symmetry": this.addKeyframe( nodeObj.node, attr, f, 0 ); break;
			default : this.addKeyframe( nodeObj.node, attr, f, -pos1 ); break;
		}
		...
		{
			var orient = node.getAttr( nodeObj.node, f, "orientation" ).doubleValue();
			this.addKeyframe( nodeObj.node, "orientation", f, orient );			
		}		
		...
		if( applyToOpposite )
		{
			...
			var flipped0 = this.getMinDistanceToProjection( orient0, projection0 );
			this.addKeyframe( handle0Node, orient0attr, f, flipped0 );
			this.addKeyframe( handle0Node, length0attr, f, length1 );			
		}
		else
		{
			// set keys as is
			...
			this.addKeyframe( handle0Node, "orientation0", f, orient0_2 );
			this.addKeyframe( handle0Node, "length0", f, length0_2 );			
		}
		...
		{		
			...
			var flipped1 = this.getMinDistanceToProjection( orient1, projection1 );				
			this.addKeyframe( handle1Node, orient1attr, f, flipped1 );
			this.addKeyframe( handle1Node, length1attr, f, length0 );			
		}
		else
		{
			// set keys as is
			...
			this.addKeyframe( handle1Node, "orientation1", f, orient1_2 );
			this.addKeyframe( handle1Node, "length1", f, length1_2 );			
		}	
	};
	...
	this.swapPoints = function( prime, opp, f, pref )
	{
		...
		if( applyToPrimal )
		{
			...
			switch( pref.axis )
			{			
				case "horizontal" : this.addKeyframe( opp.node, "offset.x", f, -primeX );
									this.addKeyframe( opp.node, "offset.y", f, primeY ); break;
																		
				case "vertical"   : this.addKeyframe( opp.node, "offset.x", f, primeX );
									this.addKeyframe( opp.node, "offset.y", f, -primeY );									
		...
		{
			// set keys as is
			this.addKeyframe( opp.node, "offset.x", f, oppX );
			this.addKeyframe( opp.node, "offset.y", f, oppY );	
		...
		{		
			...
			switch( pref.axis )
			{
				case "horizontal" : this.addKeyframe( prime.node, "offset.x", f, -oppX );
									this.addKeyframe( prime.node, "offset.y", f, oppY ); break;
																		
				case "vertical"   : this.addKeyframe( prime.node, "offset.x", f, oppX );
									this.addKeyframe( prime.node, "offset.y", f, -oppY );		
									rotate180 = false; break;
			}
		}
		else
		{
			// set keys as is
			this.addKeyframe( prime.node, "offset.x", f, primeX );
			this.addKeyframe( prime.node, "offset.y", f, primeY );			
		}
		...
		if( pref.mode == "symmetry" && pref.operation == "openEnvelope" && prime.type == "OffsetModule" )
		{
			...
			if( applyToPrimal )
			{	
				this.angleSwap( opp1Node, oppOrient1, primeOrient0, f, "orientation1", rotate180 );				
				this.addKeyframe( opp1Node, "length1", f, primeLength0 );

				// set keys as is
				this.addKeyframe( prime0Node, "orientation0", f, primeOrient0 );
				this.addKeyframe( prime0Node, "length0", f, primeLength0 );		
			}
			else
			{			
				this.angleSwap( prime0Node, primeOrient0, oppOrient1, f, "orientation0", rotate180 );		
				this.addKeyframe( prime0Node, "length0", f, oppLength1 );

				// set keys as is
				this.addKeyframe( opp1Node, "orientation1", f, oppOrient1 );
				this.addKeyframe( opp1Node, "length1", f, oppLength1 );				
			}
		}
		else
		{
			...
			{				
				...
				this.angleSwap( opp0Node, oppOrient0, primeOrient1, f, "orientation0", rotate180 );
				this.angleSwap( opp1Node, oppOrient1, primeOrient0, f, "orientation1", rotate180 );				
				
				// opposite handle length
				this.addKeyframe( opp0Node, "length0", f, primeLength1 );
				this.addKeyframe( opp1Node, "length1", f, primeLength0 );			
			}
			else
			{
				// set keys as is
				this.addKeyframe( opp0Node, "orientation0", f, oppOrient0 );
				this.addKeyframe( opp1Node, "orientation1", f, oppOrient1 );
				this.addKeyframe( opp0Node, "length0", f, oppLength0 );
				this.addKeyframe( opp1Node, "length1", f, oppLength1 );			
			}
			...
			{		
				...
				this.angleSwap( prime0Node, primeOrient0, oppOrient1, f, "orientation0", rotate180 );
				this.angleSwap( prime1Node, primeOrient1, oppOrient0, f, "orientation1", rotate180 );
				
				// primary handle length		
				this.addKeyframe( prime0Node, "length0", f, oppLength1 );
				this.addKeyframe( prime1Node, "length1", f, oppLength0 );			
			}
			else
			{
				// set keys as is
				this.addKeyframe( prime0Node, "orientation0", f, primeOrient0 );
				this.addKeyframe( prime1Node, "orientation1", f, primeOrient1 );
				this.addKeyframe( prime0Node, "length0", f, primeLength0 );
				this.addKeyframe( prime1Node, "length1", f, primeLength1 );
			}
		}
	};

	
	this.angleSwap = function( argNode, argVal, refVal, f, attr, rotate180 )
	{
		var projection = this.getProjectedAngle( refVal, rotate180 );
		var flipped = this.getMinDistanceToProjection( argVal, projection );
		this.addKeyframe( argNode, attr, f, flipped );
	};


	this.addKeyframe = function( argNode, attr, f, val )
	{
		...
		column.setEntry( col, 0, f, val.toFixed(20) );
	};
	...
	this.offsetToCenter = function( argNode, f, pref, offsetVal )
	{
		...
		column.setEntry( col, 0, f, val.toFixed(20) - offsetVal /2 );			
	};
	...
	this.applyToResting = function( argNode, f )
	{
		...
		if( attrs.length > 0 )
		{
			...
			{
				var val = node.getAttr( argNode, f, attrs[at] ).doubleValue();
				node.setTextAttr( argNode, restAttrs[at], f, val );
			}
		}
	};
