/*
	Duplicate Composite With Links v1.0

	Duplicate selected composite nodes while maintaining the link between the comp and other nodes.
...
function RIG_Duplicate_Composite_With_Links()
{
	...
	for( var idx = 0; idx < sortedCompList.length; idx++ )
	{
		...
		if( sNodeType == "COMPOSITE" )
		{
			...
			function setAttr( ogNode, newNode, attrName )
			{
			...
			{
				setAttr( sNode, newComp, compAttrs[idx2] );
			}
		}
