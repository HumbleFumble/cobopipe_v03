...
class Node(object):
	...
	def getInfoDict(self):
		...
		if self.__type == "seq":
			shot_dict = {}
			shot_dict["episode_name"] = self.__parent.getName()
			shot_dict["seq_name"] = self.__name
			# shot_dict["episode_name"], shot_dict["seq_name"] = self.__name.split("_")

		if self.__type == "shot":
			shot_dict = {}
			shot_dict["episode_name"] = self.__parent.getParent().getName()
			shot_dict["seq_name"] = self.__parent.getName()
			shot_dict["shot_name"] = self.__name
		return shot_dict
...
class RepositoryFactory(object):
	...
	def FindSequence(self, content):
		low_case = content.lower()
		re_compile = re.compile("%s" % (CC.seq_regex))
		if re_compile.search(low_case):
			return True
		else:
			return False
	...
	def data(self, index, role):
		...
		if role == QtCore.Qt.DecorationRole:
			...
			if not node.getType() == "shot":
				...
				pixmap = self.__pixmap_util.convertPathToPixmap(cache_name=f"{node.getParent().getName()}_{node.getName()}",
	...
	def data(self, index, role):
		...
		if role == QtCore.Qt.DecorationRole:
			...
			if cur_node.getType() == "shot":
				return self.pixmap_util.convertPathToPixmap(cache_name=f"{cur_node.getParent().getName()}_{cur_node.getName()}",
...
if __name__ == '__main__':
	# low_case = "Q0001_Somethine".lower()
	# re_compile = re.compile("%s" % (CC.seq_regex))
	# print(re_compile.search(low_case))
